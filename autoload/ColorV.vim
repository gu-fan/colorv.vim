...
if v:version < 700 || exists("g:colorV_loaded")
    finish
endif
let g:colorV_loaded = 1

if !has("gui_running")
    let s:ColorV_t_co=256
    set t_Co=256
endif
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
"GVAR: "{{{1 
...
function! ColorV#rgb2lab(rgb) "{{{
    ...
    if (X > 0.008856)
      ...
      let X = (7.787 * X) + (16 / 116)
    endif
    if (Y > 0.008856)
      let Y = pow(Y ,(0.3333333))
    else
      let Y = (7.787 * Y) + (16 / 116)
    endif
    if (Z > 0.008856)
      let Z = pow(Z ,(0.3333333))
    else
      let Z = (7.787 * Z) + (16 / 116)
    endif
...
function! s:delta_e_cie2000(hex1,hex2) "{{{
    ...
    let G = 0.5 * (1 - sqrt(pow(avg_C1_C2 , 7.0) / (pow(avg_C1_C2, 7.0) + pow(25.0, 7.0))))
  ...
  if s:degrees(atan2(b1,a1p)) >= 0
    let h1p = s:degrees(atan2(b1,a1p))
  else
    let h1p = s:degrees(atan2(b1,a1p)) + 360
endif
    
  if s:degrees(atan2(b2,a2p)) >= 0
    let h2p = s:degrees(atan2(b2,a2p))
  else
    let h2p = s:degrees(atan2(b2,a2p)) + 360
endif

  if abs(h1p - h2p) > 180
    let avg_Hp = (h1p + h2p + 360) / 2.0
  else
    let avg_Hp = (h1p + h2p) / 2.0
endif

  let T = 1 - 0.17 * cos(s:radians(avg_Hp - 30)) + 0.24 * cos(s:radians(2 * avg_Hp)) + 0.32 * cos(s:radians(3 * avg_Hp + 6)) - 0.2  * cos(s:radians(4 * avg_Hp - 63))

  let diff_h2p_h1p = h2p - h1p
  if abs(diff_h2p_h1p) <= 180
    let delta_hp = diff_h2p_h1p
  elseif (abs(diff_h2p_h1p) > 180) && (h2p <= h1p)
    let delta_hp = diff_h2p_h1p + 360
  else
    let delta_hp = diff_h2p_h1p - 360
endif
  ...
  let delta_Hp = 2 * sqrt(C2p * C1p) * sin(s:radians(delta_hp) / 2.0)
  let S_L = 1 + ((0.015 * pow(avg_Lp - 50, 2)) / sqrt(20 + pow(avg_Lp - 50, 2.0)))
  let S_C = 1 + 0.045 * avg_C1p_C2p
  let S_H = 1 + 0.015 * avg_C1p_C2p * T

  let delta_ro = 30 * exp(-(pow(((avg_Hp - 275) / 25), 2.0)))
  let R_C = sqrt((pow(avg_C1p_C2p, 7.0)) / (pow(avg_C1p_C2p, 7.0) + pow(25.0, 7.0)))
  let R_T = -2 * R_C * sin(2 * s:radians(delta_ro))

  let delta_E = sqrt(pow(delta_Lp /(S_L * Kl), 2) + pow(delta_Cp /(S_C * Kc), 2) + pow(delta_Hp /(S_H * Kh), 2) + R_T * (delta_Cp /(S_C * Kc)) * (delta_Hp / (S_H * Kh)))
...
def color_norm(c): # {{{
  ...
  #}}}
class color: # {{{ "{{{
  def __init__(self, colorstring, g=None, b=None): # {{{
  ...
  def tolab(self): #{{{
    ...
    # 0.6237966 0.2848  0.8906808
    # 0.8544388 0.65793 0.9621453
    #Lab 60.31988 98.254 -60.84306
    if (X > 0.008856):
      # X = X ** (1/3)
      X = X ** (0.333333333)
    else:
...
def delta_e_cie2000(color1, color2): # {{{
  ...
  elif (abs(diff_h2p_h1p) > 180) and (h2p <= h1p):
...
val = color(vim.eval("hex1")).termcolor
vim.command("let tmcolor = "+str(val))
# vim.command("let tmcolor = \"" + termcolor[val]+"\"")

EOF
    " let tmcolor=substitute(tmcolor,"#","","")
    return tmcolor
endfunction "}}}
...
function! s:hi_color(hl_grp,hl_fg,hl_bg,...) "{{{
    ...
    if has("gui_running")
        exec "hi ".hl_grp." guifg=".hl_fg." guibg=".hl_bg.hl_fmt
    else
        exec "hi ".hl_grp." ctermfg=".hl_fg." ctermbg=".hl_bg.hl_fmt
    endif
endfunction "}}}
...
function! ColorV#Win(...) "{{{
    ...
    if bufexists(g:ColorV.name) 
        ...
        if winnr>0 && bufname(nr)==g:ColorV.name
            if bufwinnr('%') ==winnr
            	if g:ColorV_win_pos=~'bo\%[tright]'
                    exec winnr."wincmd J"
                elseif g:ColorV_win_pos=~'to\%[pleft]'
                    exec winnr."wincmd K"
                endif
                if expand('%') !=g:ColorV.name
                    call s:warning("Not [ColorV] buffer")
...
function! s:map_define() "{{{
    ...
    nmap <silent><buffer> geh :call ColorV#gen_win(g:ColorV.HEX,"Hue")<cr>
    nmap <silent><buffer> ges :call ColorV#gen_win(g:ColorV.HEX,"Saturation")<cr>
    nmap <silent><buffer> gev :call ColorV#gen_win(g:ColorV.HEX,"Value")<cr>
    nmap <silent><buffer> gea :call ColorV#gen_win(g:ColorV.HEX,"Analogous")<cr>
    nmap <silent><buffer> geq :call ColorV#gen_win(g:ColorV.HEX,"Square")<cr>
    nmap <silent><buffer> gen :call ColorV#gen_win(g:ColorV.HEX,"Neutral")<cr>
    nmap <silent><buffer> gec :call ColorV#gen_win(g:ColorV.HEX,"Clash")<cr>
    nmap <silent><buffer> gep :call ColorV#gen_win(g:ColorV.HEX,"Split-Complementary")<cr>
    nmap <silent><buffer> ge1 :call ColorV#gen_win(g:ColorV.HEX,"Monochromatic")<cr>
    nmap <silent><buffer> gem :call ColorV#gen_win(g:ColorV.HEX,"Monochromatic")<cr>
    nmap <silent><buffer> ge2 :call ColorV#gen_win(g:ColorV.HEX,"Complementary")<cr>
    nmap <silent><buffer> get :call ColorV#gen_win(g:ColorV.HEX,"Triadic")<cr>
    nmap <silent><buffer> ge3 :call ColorV#gen_win(g:ColorV.HEX,"Triadic")<cr>
    nmap <silent><buffer> ge4 :call ColorV#gen_win(g:ColorV.HEX,"Tetradic")<cr>
    nmap <silent><buffer> ge5 :call ColorV#gen_win(g:ColorV.HEX,"Five-Tone")<cr>
    nmap <silent><buffer> ge6 :call ColorV#gen_win(g:ColorV.HEX,"Six-Tone")<cr>
...
function! s:txt2hex(txt) "{{{
    ...
    while rnd<=20
        for [fmt,pat] in items(s:fmt)
            if text=~ pat
                let pat_idx{idx}=match(text,pat)
                let pat_str{idx}=matchstr(text,pat)
                " error with same hex in one line?
                " it will match the first one
                let pat_oidx{idx}=match(textorigin,pat_str{idx})
                let pat_len{idx}=len(pat_str{idx})
                let text=strpart(text,0,pat_idx{idx})
                            \.strpart(text,pat_len{idx}+pat_idx{idx})

                let hex_dict[fmt]=exists("hex_dict[fmt]") ? hex_dict[fmt] : []
                call add(hex_dict[fmt]
                            \,[pat_str{idx},pat_oidx{idx}
                            \,pat_len{idx},pat_str{idx},fmt])
                let idx+=1
            endif
        endfor
        if old_dict==hex_dict          
            break
        else
            let old_dict=deepcopy(hex_dict,1)
        endif
        let rnd+=1
    endwhile
    ...
    if exists("s:ColorV.change_word") && s:ColorV.change_word ==1
        ...
        if s:ColorV.word_bufnr==bufnr('%') && s:ColorV.word_pos==getpos('.')
            ...
            elseif s:ColorV.is_in=="line"
                ...
                call s:error("Don't know what to change.")
                return -1
            endif
            ...
            if exists("s:ColorV.word_list[0]")
                ...
                call s:error("Could not find a color under cursor.")
