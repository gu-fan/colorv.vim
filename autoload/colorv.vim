...
let g:ColorV.listname="_ColorV_List_"
...
" endif
if !exists('g:ColorV_cursor_found')
    let g:ColorV_cursor_found=0
endif
"let preview color became massive
if !exists('g:ColorV_prev_mass')
    let g:ColorV_prev_mass=0
endif
if !exists('g:ColorV_view_name')
    let g:ColorV_view_name=1
endif
if !exists('g:ColorV_input_gen_step')
    let g:ColorV_input_gen_step=1
endif

if !exists('g:ColorV_gen_def_nums')
    let g:ColorV_gen_def_nums=20
endif
if !exists('g:ColorV_gen_def_step')
    let g:ColorV_gen_def_step=5
endif
if !exists('g:ColorV_gen_def_type')
    let g:ColorV_gen_def_type="Hue"
endif
if !exists('g:ColorV_cursor_back')
    let g:ColorV_cursor_back=1
endif
if !exists('g:ColorV_name_approx')
    let g:ColorV_name_approx=5
endif
if !exists('g:ColorV_view_block')
    let g:ColorV_view_block=0
endif
if !exists('g:ColorV_color_space')
    let g:ColorV_color_space="hsv"
endif
"}}}
"SVAR: {{{1
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
let s:ColorV.WinSize = 5
let s:ColorV.WinPos = "right"
if g:ColorV_color_space==?"hls"
    let s:space=exists("s:space") ? s:space : "hls"
else
    let s:space=exists("s:space") ? s:space : "hsv"
endif

let s:his_cpd_list=exists("s:his_cpd_list") 
            \ ? s:his_cpd_list : []
let s:ColorV.history_set=exists("s:ColorV.history_set")
            \ ? s:ColorV.history_set : ['ff0000']
...
let s:tmclr16_dict= {
            \0: ["#000000","Black"], 
            \1: ["#000080","DarkBlue"],2: ["#008000","DarkGreen"], 3: ["#008080","DarkCyan"], 
            \4: ["#800000"," DarkRed"], 5: ["#800080","DarkMagenta"], 6: ["#808000","DarkYellow"], 
            \7: ["#c0c0c0","LightGray"], 8: ["#808080","DarkGray"], 9: ["#ff0000","Blue"], 
            \10: ["#00ff00","Green"], 11: ["#00ffff","Cyan"], 12: ["#ff0000","Red"], 
            \13: ["#ff00ff","Magenta"], 14: ["#ffff00","Yellow"], 15: ["#ffffff","White"]
            \}
"terminal color 256
let s:tmclr_dict = { 
...
def rgb2hex(rgb):
    ...
    return '%06X'%((rgb[0] << 16) + (rgb[1] << 8) + rgb[2]) #}}}
#{{{ hsv
def rgb2hsv(rgb): 
    r,g,b=rgb
...
EOF
    ...
    let [r,g,b]=[r/100.0,g/100.0,b/100.0]
...
EOF
    ...
    if s==0
        ...
        r= h2rgb(var_1,var_2,(h+1.0/3))
        g= h2rgb(var_1,var_2,h)
        b= h2rgb(var_1,var_2,(h-1.0/3))
    endif
...
function! colorv#yuv2rgb(yuv) "{{{
    ...
    if R>1 || G>1 || B>1 || R<0 || G<0 || B<0
    	" echom "Invalid RGB for with YUV" string(a:yuv)
        let R = R>1 ? 1 : R<0 ? 0 : R
        let G = G>1 ? 1 : G<0 ? 0 : G
        let B = B>1 ? 1 : B<0 ? 0 : B
    endif
...
EOF
    ...
    if Y>100||I>100||Q>100 || Y<-100||I<-100||Q<-100
        let Y=s:fmod(Y,100.0)
...
"vim is slower than python code.
"So use python by default.
    if exists("a:1") && a:1 =="CHECK_TERMCO" && (&t_Co == 16 || &t_Co ==8)
        if has("python")
            try
...
function! colorv#rgb2term(rgb) "{{{
let hex=colorv#rgb2hex(a:rgb)
let tmcolor=colorv#hex2term(hex)
return tmcolor
endfunction "}}}
...
python << EOF
termcolor = vim.eval("s:tmclr_dict")
termcolor16 = vim.eval("s:tmclr16_dict")
tmclr_dict = { 
            16: '000000', 17: '00005f', 18: '000087',
            19: '0000af', 20: '0000d7', 21: '0000ff',
            22: '005f00', 23: '005f5f', 24: '005f87',
...
function! s:v_hex2term16(hex) "{{{
    ...
    for c in range(0,15)
        let [r2,g2,b2] = colorv#hex2rgb(s:tmclr16_dict[c][0])
    ...
    return s:tmclr16_dict[best_match][1]
endfunction "}}}
...
function! s:v3_hex2term(hex) "{{{
    ...
    for c in range(n1,n1+35)+range(232,255)
        let [r2,g2,b2] = colorv#hex2rgb(s:tmclr_dict[c])
        let d = abs(r1-r2)+abs(g1-g2)+abs(b1-b2)
    	if d < smallest_distance
    	    let smallest_distance = d
    	    let best_match = c
...
for c in range(n1, n1+36)+range(232,256):
...
vim.command("return "+str(best_match))
EOF
endfunction "}}}
...
def draw_palette(H,he,wi,*off): #{{{
    ...
    while line <= he:
        ...
        while col<=wi:
            ...
            hi_grp="cvpal_"+hex
            hi_cmd="call s:hi_color(\""+hi_grp+"\",\""+hex+"\",\""+hex+"\")"
            cmd_list.append(hi_cmd)
            pos_ptn="\\%"+str(line+y_off)+"l\\%"+str(col+x_off)+"c"
            match_cmd="let s:pallet_dict['"+hi_grp+"']=matchadd('"+hi_grp+"','"+pos_ptn+"')"
            cmd_list.append(match_cmd)
            col+=1
        line+=1
    for cmd in cmd_list:
    	vim.command(cmd)
    vim.command("let s:pal_clr_list="+str(pal_clr_list))
#}}}
...
" TODO: 110823  draw multiple multi block
...
function! s:draw_history_block(hex) "{{{
    ...
    let len=len(s:ColorV.history_set)
    let s:his_color2= len >2 ? s:ColorV.history_set[len-3] : 'ffffff'
    let s:his_color1= len >1 ? s:ColorV.history_set[len-2] : 'ffffff'
    let s:his_color0= len >0 ? s:ColorV.history_set[len-1] : hex
...
function! s:draw_history_copy() "{{{
    ...
    let clr_list=[]
    for i in range(17)
        let cpd_color= len>i ? s:his_cpd_list[-1-i] : 333333
...
function! s:hi_color(hl_grp,hl_fg,hl_bg,...) "{{{
    ...
    if has("gui_running")
    	...
    	if hl_fg=~'\x\{6}'
            let hl_fg=colorv#hex2term(hl_fg)
        endif
    	if hl_bg=~'\x\{6}'
            let hl_bg=colorv#hex2term(hl_bg)
        endif
        if exists("a:1")
        ...
        let hl_fg = hl_fg=="" ? "" : " ctermfg=".hl_fg
        let hl_bg = hl_bg=="" ? "" : " ctermbg=".hl_bg
    endif
...
function! s:update_his_set(hex) "{{{
    ...
    if exists("s:skip_his_block") && s:skip_his_block==1
        ...
        if get(s:ColorV.history_set,-1)!=hex
            call add(s:ColorV.history_set,hex)
        endif
    endif
endfunction "}}}
...
function! s:hi_misc() "{{{
    ...
    " call s:hi_color("arrowChose","fg","bg","Bold,reverse")
    ...
    " call s:hi_color("arrow_invis","bg","bg")
    " let arrow_ptn='\%<6l_
    "             \\|N:'
    ...
    " if g:ColorV_show_star==1
        ...
        " let bg= g:ColorV.HEX
    " let fg= g:ColorV.HSV.V<50 ?  "bbbbbb" : "333333"
    ...
    let fg= s:rlt_clr(bg)
    call s:hi_color("starPos",fg,bg,"Bold")
    " TODO: Missing * sometimes. why?
    " the position of s:pal_H is changing.
    let star_ptn='\%<'.(s:pal_H+1+s:poff_y).'l\%<'.
                \(s:pal_W+1+s:poff_x).'c\*'
    let s:misc_dict["starPos"]=matchadd("starPos",star_ptn,40)
    " endif
    if s:mode=="min"
        " let fg= g:ColorV.HSV.V<50 ?  "cccccc" : "222222"
        " let c = 20 - g:ColorV.HSV.S/5
        " let c = c==20 ? 19 : c
        let [l,c]=s:get_bar_pos()
        let bg= s:satline_list[c-1]
        let fg= s:rlt_clr(bg)
        " exe "hi starPos guibg=#".bg." guifg=#".fg." gui=Bold"
        call s:hi_color("barPos",fg,bg,"Bold")
        let bar_ptn='\%2l\%<'.
                    \(s:pal_W+1+s:poff_x).'c+'
        let s:misc_dict["barPos"]=matchadd("barPos",bar_ptn,20)
    endif
    ...
    if s:mode!="min" 
        " if s:mode!="mid" 
        "     let tip_ptn='\%6l\%>23c\%<55c'
        " else
        let tip_ptn='\%'.(s:pal_H+1).'l\%>21c\%<60c'
        let q_ptn='\%'.(s:pal_H+1).'l\%>21c\%<60c[mM?]'
        " endif
        " call s:hi_color("tipPos",fg,bg,"Bold")
        call s:hi_link("tipPos","Title")
        call s:hi_link("qPos","ModeMsg")
        let s:misc_dict["tipPos"]=matchadd("tipPos",tip_ptn)
        let s:misc_dict["qPos"]=matchadd("qPos",q_ptn,25)
    endif
endfunction "}}}
...
function! s:draw_text(...) "{{{
    ...
    if s:mode=="max"  
        ...
        let line[3]=s:line("H:".H."  L:".L."  S:".S,22)
            ...
            " let line[3]=s:line("G:".g."   L:".L,24)
            " let line[4]=s:line("B:".b."   S:".S,24)
            " let line[2]=s:line("R:".r."   H:".h,24)
            " let line[3]=s:line("G:".g."   S:".s,24)
            " let line[4]=s:line("B:".b."   V:".v,24)
        let line[4]=s:line("Y ".Y."  I".I."  Q".Q,22)
        let line[s:pal_H]=s:line("<F1>:help <Enter>:edit q:exit    M?",22)
    elseif s:mode=="mid"
        let line[0]=s:line("ColorV ".g:ColorV.ver,3)
        " let line[0]=s:line_sub(line[0],"N:",40)
    ...
    elseif s:mode=="min"
        let line[0]=s:line("ColorV ".g:ColorV.ver,3)
        let line[0]=s:line_sub(line[0],"Hex:".hex,22)
        " let line[0]=s:line_sub(line[0],"_?",39)
        " let line[0]=s:line_sub(line[0],"N:",40)
        let line[1]=s:line("R:".r."  G:".g."  B:".b,22)
        if s:space=="hls"
...
function! s:get_star_pos() "{{{
    ...
    elseif s:mode=="min"
        " DONE: there should be 3(or 2 if with dynamic hueline) stars here , 
        ...
        " echoe "c" string(c) "s" string(w_step) "v" v
        if c>=s:pal_W+s:poff_x
            let c= s:pal_W+s:poff_x
        endif
    endif
...
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
function! colorv#win(...) "{{{
" DONE: 110819  wierd window error (new and exit.)

    let splitLocation = s:ColorV.WinPos == "left" ? "topleft " : "botright "
    let splitSize = s:ColorV.WinSize

    if !exists('t:ColorVBufName')
        let t:ColorVBufName = g:ColorV.name
        silent! exec splitLocation . ' ' . splitSize . ' new'
        " Error in pwd:"/" with terminal mode
        silent! exec "edit ~/" . t:ColorVBufName
    else
    	if s:isOpen()
            call s:exec(s:getWinNum() . " wincmd w")
    ...
    if exists("a:2") 
        ...
        if exists("hex_list[0][0]")
            ...
            " if !exists("a:3") 
                call s:echo("Use [".hex."]") 
            " elseif exists("a:3") && a:3!="SILENT"
            "     " call s:echo("Use [".hex."]") 
            "     call s:echo("Use [".hex."] Format:".a:3) 
            " endif
        else
            let hex = exists("g:ColorV.HEX") ? g:ColorV.HEX : "ff0000"
            let s:skip_his_block=1
            " if !exists("a:3") || a:3!="SILENT"
...
if g:ColorV_debug==1
    ...
    if has("python")
        call colorv#timer("s:p_draw_hueline",[1])
    else
        call s:draw_hueline(hex)
    endif
    if s:mode == "min"
    ...
    elseif s:mode == "max"
    	...
    	if has("python")
            call colorv#timer("s:p_draw_palette_hex",[hex])
        else
            call s:draw_palette_hex(hex)
        endif
        let l:win_h=s:mid_h
    ...
    if s:mode == "min"
        ...
        if g:ColorV.HSV.S==0
            ...
            if has("python")
                call s:p_draw_palette_hex(prv_hex)
            else
...
function! s:map_define() "{{{
    ...
    " nmap <silent><buffer> e :call <SID>edit_at_cursor()<cr>
    " nmap <silent><buffer> <kEnter> :call <SID>edit_at_cursor()<cr>
    " nmap <silent><buffer> <kEnter> :call <SID>edit_at_cursor()<cr>
...
function! colorv#exit() "{{{
    if !s:isOpen()
        " throw "ColorV.NoWinFoundError: no ColorV is open"
        " return
    else
        if winnr("$") != 1
            call s:exec(s:getWinNum() . " wincmd w")
    ...
    if !s:isOpen("t:ColorVListBufName")
        " throw "ColorV.NoWinFoundError: no ColorV is open"
        " return
    else
        if winnr("$") != 1
            call s:exec(s:getWinNum("t:ColorVListBufName") . " wincmd w")
            close
            call s:exec("wincmd p")
        else
            close
...
print "Select a color and press OK."
...
function! s:set_in_pos(...) "{{{
    ...
    elseif s:mode=="max" && l==s:his_cpd_rect[1] &&
                \ c>=s:his_cpd_rect[0] 
                \ && c<=(s:his_cpd_rect[0]+s:his_cpd_rect[2]*17-1)  
        for i in range(17)
    ...
    elseif l==s:pal_H+1 && c==56 && matchstr(getline(s:pal_H+1),'.',55,1)=="?"
        call s:echo_tips()
        return -1
    elseif l==s:pal_H+1 && c==55 && matchstr(getline(s:pal_H+1),'.',54,1)=~?"m"
        if s:mode=="min"
            call ColorV#win("max")
        elseif s:mode=="max"
            call ColorV#win("mid")
...
function! s:edit_at_cursor(...) "{{{
    "setl ma
    " let position=exists("a:1") && a:1!=-1 ? a:1 : b:arrowck_pos
    let tune=exists("a:2") ? a:2 == "+" ? 1 : a:2 == "-" ? -1  : 0  : 0
    " call s:draw_arrow(position)
    let clr=g:ColorV
    let hex=clr.HEX
    let [r,g,b]=[clr.RGB.R,clr.RGB.G,clr.RGB.B]
    let [h,s,v]=[clr.HSV.H,clr.HSV.S,clr.HSV.V]
        ...
        for [fmt,pat] in items(s:fmt)
            if text=~ pat
                ...
                elseif fmt=="RGB" || fmt =="RGBA"
                    ...
                    if r>255 || g >255 || b > 255
                        call s:error("RGB input out of boundary")
                        return
                    endif
                    let hex = colorv#rgb2hex([r,g,b])
                    let list=[hex,p_oidx{idx},p_len{idx},p_str{idx},fmt]
...
def approx2(hex1,hex2,rate):
    r1,g1,b1 = hex2rgb(hex1)
    r2,g2,b2 = hex2rgb(hex2)
    t=5
    if r2+t>=r1 and r1>=r2-t and g2+t>=g1 and g1>=g2-t \
                and b2+t>=b1 and b1>=b2-t:
    	return 1
    else:
    	return 0
EOF
...
python << EOF
best_match = 0
smallest_distance = 20000000

hex1 = vim.eval("a:hex")
t= vim.eval("s:aprx_name")
if vim.eval("list")=="X11":clr_list=clr_listX11
else:clr_list=clr_listW3C
r1,g1,b1 = int(hex1[0:2],16),int(hex1[2:4],16),int(hex1[4:6],16)

for idx in range(len(clr_list)):
    hex2 = clr_list[idx][1]
...
if smallest_distance == 0:
    vim.command("return \""+clr_list[best_match][0]+"\"")
...
EOF
    ...
    "find the smallest_distance in it
    let best_match=0
    let smallest_distance = 20000000
    let hex1=a:hex
    let [r1,g1,b1] = ["0x".hex1[0:1],"0x".hex1[2:3],"0x".hex1[4:5]]
    for idx in range(len(clr_list))
    	let hex2=s:fmt_hex(clr_list[idx][1])
    ...
    if smallest_distance == 0
    	return clr_list[best_match][0]
    elseif smallest_distance <= s:aprx_name*5
    	return clr_list[best_match][0]."~"
    elseif smallest_distance <= s:aprx_name*10
    	return clr_list[best_match][0]."~~"
    else
    	return ""
    endif
    ...
    if exists("s:ColorV.change_word") && s:ColorV.change_word ==1
        ...
        if s:ColorV.word_bufnr==bufnr('%') && s:ColorV.word_pos==getpos('.')
            ...
            if from_pat!= s:ColorV.word_pat
                call s:error("Not the right word to change.")
                return -1
            endif "}}}
...
function! colorv#cursor_win(...) "{{{
    ...
    elseif exists("a:1") && a:1==3
        ...
        let list=s:gen_win_list(hex,type,nums,step)
    ...
    if g:ColorV_word_mini==1
    	" if exists("l:silent") && l:silent==1
            call colorv#win("min",hex,"SILENT")
        " elseif exists("clr_fmt")
        "     call colorv#win("min",hex,clr_fmt)
        " else
...
function! colorv#list_win(...) "{{{
    ...
    if !exists('t:ColorVListBufName')
    	...
    	if s:isOpen("t:ColorVListBufName")
            ...
            silent! exec splitLocation . ' ' . splitSize . ' vsplit'
...
"
    ...
    let type=a:mode
    let [y,u,v]=colorv#rgb2yiq(colorv#hex2rgb(a:hex))
    " use HSV to justify it
    let [H,S,V]=colorv#hex2hsv(a:hex)
    let hex_list=[]
    if type==?"Triadic"
        let list=s:circle_coord_gen(u,v,4)
        for i in range(4)
            let hex=colorv#rgb2hex(colorv#yiq2rgb([y,list[i][0],list[i][1]]))
            call add(hex_list,hex)
        endfor
    elseif type==?"Complementary"
        let [y1,u1,v1]=[y,-u,-v]
        let hex1=colorv#rgb2hex(colorv#yiq2rgb([y1,u1,v1]))
        call add(hex_list,hex1)
    elseif type=="Value"
        "v+
        " let v{i}= v+step*i<=100 ? v+step*i : 100
        let num= exists("a:1") ? a:1 : 10
        let step= exists("a:2") ? a:2 : 10
        for i in range(num)
            let V{i}=V+step*i
            let V{i} = V{i} >=100 ? 100 : V{i} <= 0 ? 0 : V{i}
            let hex{i}=colorv#hsv2hex([H,S,V{i}])
            let [y{i},u{i},v{i}]=colorv#rgb2yiq(colorv#hex2rgb(hex{i}))
            " XXX:This will cause it became the same color..
            let hex{i} = colorv#rgb2hex(colorv#yiq2rgb([y,u{i},v{i}]))
            call add(hex_list,hex{i})
        endfor
    elseif type=="Split-Complementary"
        " XXX: should be h+150 h+60
        let hex_list=colorv#yuv_list_gen(a:hex,"Hue",3)
    elseif type=="Clash"
        " XXX: should be h+90 h+180
        let hex_list=colorv#yuv_list_gen(a:hex,"Hue",3)
    elseif type==?"Six-Tone"
        " DONE: 110820  use arc to do this
        let list=s:circle_coord_gen(u,v,6)
        for i in range(6)
            let hex{i}=colorv#rgb2hex(colorv#yiq2rgb([y,list[i][0],list[i][1]]))
            call add(hex_list,hex{i})
        endfor
    elseif type==?"Five-Tone"
        let hex_list=colorv#yuv_list_gen(a:hex,"Hue",5)
    elseif type=="Tetradic" || type=="Rectangle"
            "h+60,h+120,...
        let hex_list=colorv#yuv_list_gen(a:hex,"Hue",4)
    elseif type=="Triadic"
        let hex_list=colorv#yuv_list_gen(a:hex,"Hue",3)
    elseif type=="Analogous"
        let hex_list=colorv#yuv_list_gen(a:hex,"Hue",12)
    elseif type==?"Hue"
        let num= exists("a:1") ? a:1 : 20
        " " let step= exists("a:1") ? a:1 : 30
        " let list=s:circle_coord_gen(u,v,num)
        " for i in range(num)
        "     let hex=colorv#rgb2hex(colorv#yiq2rgb([y,list[i][0],list[i][1]]))
        "     call add(hex_list,hex)
        " endfor
        for i in range(num)
            let H{i}=H+i*360.0/num
    ...
    elseif type==?"Neutral"
        let hex_list=colorv#yuv_list_gen(a:hex,"Hue",24)
    elseif type==?"Luma"
        let num= exists("a:1") ? a:1 : 10
        let step= exists("a:2") ? a:2 : 10
        for i in range(num)
            ...
            let hex=colorv#rgb2hex(colorv#yiq2rgb([y{i},u,v]))
            " echoe string(hex) i num step
            call add(hex_list,hex)
        endfor
    " else
...
function! colorv#yuv_list_win(color,mode,...) "{{{
    let type = a:mode
    if exists("a:1")
        let genlist=colorv#yuv_list_gen(a:color,type,a:1)
    else
        let genlist=colorv#yuv_list_gen(a:color,type)
    endif
    let list=[]
    call add(list,["YUV ".type." Circle",'======='])
    let i=0
    for hex in genlist
...
function! colorv#list_gen(hex,...) "{{{
    ...
    for i in range(nums)
        ...
        elseif type=="LessSatur"
            "s+step v+step
            " let v{i}= v+step*i<=100 ? v+step*i : 100
            " let s{i}= s+step*i<=100 ? s+step*i : 100
            let s{i}=s-abs(step*i)
            let v{i}=v+step*i
            let s{i} = s{i} >=100 ? 100 : s{i} <= 0 ? 0 : s{i}
            let v{i} = v{i} >=100 ? 100 : v{i} <= 0 ? 0 : v{i}
            let hex{i}=colorv#hsv2hex([h,s{i},v{i}])
